...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def judge(text):
        words = set(split(lower(remove_punctuation(text))))
        for x in topic:
            if x in words:
                return True
        return False
...
def accuracy(typed, reference):
    ...
    if total == 0:
        return 0.0
    for i, t in zip(typed_words, reference_words):
        correct += (i == t)
...
def wpm(typed, elapsed):
    ...
    words_per_min = 60 / elapsed * word_num
...
def shifty_shifts(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    if len(start) == 0 or len(goal) == 0: return abs(len(start) - len(goal))
    else:
        if start[0] != goal[0]:
            if limit == 0: return 1
            else: return 1 + shifty_shifts(start[1:], goal[1:], limit - 1)
        else: return shifty_shifts(start[1:], goal[1:], limit)
    # END PROBLEM 6
...
def fastest_words(game):
    ...
    for word_index in range(len(all_words(game))):
        min_time = float("inf")
        for player_num in range(len(all_times(game))):
            if time(game, player_num, word_index) < min_time:
                min_time = time(game, player_num, word_index)
                record_pos = player_num
        lst[record_pos] += [word_at(game, word_index)]
    return lst
    # END PROBLEM 10
...
def key_distance_diff(start, goal, limit):
    ...
    if len(start) == 0 or len(goal) == 0:
        ...
        add_step = 1 + key_distance_diff(start, goal[1:], limit - 1)
        delete_step = 1 + key_distance_diff(start[1:], goal, limit - 1)
        key = key_distance[(start[0], goal[0])]
        substitute_step = key + key_distance_diff(start[1:], goal[1:], limit - 1)
        return min(min(add_step, delete_step), substitute_step)

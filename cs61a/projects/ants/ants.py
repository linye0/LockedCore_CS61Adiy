    ...
    def nearest_bee(self, beehive):
        ...
        def find_helper(place, distance):
            ...
            if place.bees:
                ...
                return find_helper(place.entrance, distance + 1)
        place = self.place
        distance = 0
        while place is not beehive and distance < self.min_range:
            place = place.entrance
            distance += 1
        find = find_helper(place, distance)
        return None if find is None else rANTdom_else_none(find.bees) # REPLACE THIS LINE
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.place is not place:
            dealt_damage = amount + self.damage
        else:
            dealt_damage = amount
        for bee in place.bees:
            if bee.armor > dealt_damage:
                remain_bees.append(bee)
        for bee in place.bees.copy():
            bee.reduce_armor(dealt_damage)
        place.bees = remain_bees
        # END Problem 5
...
class BodyguardAnt(ContainerAnt):
    ...
    # BEGIN Problem 9
    implemented = True
    def __init__(self, armor = 2):   # Change to True to view in the GUI
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if self.is_true:
            ...
            p = self.place.exit
            while isinstance(p, AntHomeBase) == False:
                if p.ant:
                    if p.ant.is_container:
                        if p.ant.contained_ant:
                            if p.ant.contained_ant.inspired == False:
                                p.ant.contained_ant.damage *= 2
                                p.ant.contained_ant.inspired = True
                    if p.ant.inspired == False:
                        p.ant.damage *= 2
                        p.ant.inspired = True
                p = p.exit
        else:

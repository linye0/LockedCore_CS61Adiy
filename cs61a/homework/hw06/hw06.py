...
class VendingMachine:
    ...
    def vend(self):
        if self.stock == 0:
            ...
            self.stock -= 1
            flag = self.balance - self.price
            self.balance = 0
            if flag:
                return 'Here is your {0} and ${1} change.'.format(self.name, flag)
            else:
...
def is_bst(t):
    ...
    if t.is_leaf():
        ...
        if len(t.branches) == 1:
            flag = t.label >= bst_max(t.branches[0]) or t.label < bst_min(t.branches[0])
            return flag and is_bst(t.branches[0])
        else:
            flag = t.label >= bst_max(t.branches[0]) and t.label < bst_min(t.branches[1])
            return flag and all([is_bst(b) for b in t.branches])

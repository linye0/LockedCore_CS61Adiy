...
def make_withdraw(balance, password):
    ...
    """
    wrong_lst = []
    def withdraw(amount, type_in_password):
        nonlocal wrong_lst, balance
        if len(wrong_lst) >= 3:
                return f"Too many incorrect attempts. Attempts: {wrong_lst}"
        if type_in_password == password:
            if balance < amount:
                return 'Insufficient funds'
            else:
                balance -= amount
